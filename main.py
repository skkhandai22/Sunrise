...
def rf_predict(age,type_injury,location,reason,bool_fall_risk,severity,gender):
    ...
    if severity=='Sentinel event':
        severity=0
    elif severity=='adverse event':
        severity=1
    else:
        severity=2
...
def main():
    ...
    if file_option=="data.csv":
        ...
        if selection=="Prediction":
            ...
            reason=st.selectbox('Reason for Incident',('Loss of balance', 'Trip', 'Mental confusion', 'Slip','Hypotension', 'Motor Deficit', 'Syncope', 'Dizziness','Muscle weakness', 'Equipment', 'Human error', 'Sedation','Mental confusion '))
            bool_fall_risk=st.radio('Select if any involvement of medication with fall risk',('Yes','No'))
            severity=st.select_slider('Select Severity of incident',('Sentinel','Adverse','Serious'))

            col1, col2, col3,col4,col5,col6,col7,col8,col9 = st.columns(9)
            if col5.button('Check'):
                result=rf_predict(age,type_injury,location,reason,bool_fall_risk,severity,gender)
                st.success('The person has {} fall risk level'.format(result) )
